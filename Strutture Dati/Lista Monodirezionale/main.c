...
Node* Node_create(int element, Node* next) {
...
void List_insertHead(List list, int element) {  // O(1)
    Node* newNode = Node_create(element, list->head);
...
void List__insertOrdered(Node** nodePtr, int element, int order) {
    int currentElement = (*nodePtr)->element;
    ...
    if (shouldIInsertBeforeCurrentElement) {
        Node* newNode = Node_create(element, *nodePtr);
        *nodePtr = newNode;
    } else {
...
void List__printReversed(Node* node) {
    if (node) {
        if (node->next) {
            List__printReversed(node->next);
            printf(" <- %d", node->element);
        } else {
            printf("%d", node->element);
        }
...
void List_deleteTail(List list) {  // Warning: Teta(n)
    if (list->head == NULL) {      // empty list
        return;
    }
    ...
    if (listLength == 1) {
        ...
        // at the beginning:
        // - *nodePtr is the head
        // - (*nodePtr)->next is the second element
        // - (*nodePtr)->next->next is the third element || NULL
        while (node->next->next != NULL) {
            node = node->next;
        }

        // now node->next is the tail
        // node is the node before the tail, the new tail
        Node_free(&node->next);
        list->tail = node;
    }
...
void List__reverse(Node** nodePtr, Node* previous) {
    if (*nodePtr == NULL) {  // list with 0 elements
        return;
    }

    if ((*nodePtr)->next == NULL) {
        if (previous != NULL) {           // list with more than one element
    ...
    List__reverse(&(*nodePtr)->next, *nodePtr);
